...
module.exports = function(connection, authenticat) {
  ...
  htAuthRouter.get('/trigger_ht_auth', (req, res) => {
    ...
    newUser.save((err, user) => {
      ...
      var url = 'https://chpp.hattrick.org/oauth/request_token.ashx';

      request.get({url: url, oauth: oauth}, function (err, response, body) {
        var req_data = qs.parse(body);
        var uri = 'https://chpp.hattrick.org/oauth/authorize.aspx' + '?' + qs.stringify({oauth_token: req_data.oauth_token});

        user.htTokenSecret = req_data.oauth_token_secret;
        user.save((err, user) => {
          res.redirect(uri);
        });
      });
  ...
  htAuthRouter.get('/ht_auth/:userID', (req, res) => {
    User.findById(req.params.userID, (err, user) => {
      ...
      var url = 'https://chpp.hattrick.org/oauth/access_token.ashx';
      request.get({ url: url, oauth: oauth }, function (err, response, body) {
        var access_data = qs.parse(body);
        ...
        request.get({ url: teamUrl, oauth: oauth }, function (err, response, body) {
